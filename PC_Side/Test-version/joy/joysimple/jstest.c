...
*/
	...
	effect.id = -1;
	effect.u.rumble.weak_magnitude =0xC000;// floor(0.9 * (float)0xffff);
	effect.u.rumble.strong_magnitude = 0xFFFF; //floor(0.9 * (float)0xffff);
	effect.replay.length = 5000;// floor(10 * 1000);
	effect.replay.delay = 0;

	if (ioctl(fd, EVIOCSFF, &effect) < 0) {
		//printf("Mallllllllllll\n");  // Non blocking Test
		return;
	}
